...
public class BetService {
    ...
    public Paris generateBet(BigInteger bet, CourseComplete course, Course c){
        ...
        log.debug("Bet service Generate");
        ...
        if (lastParis != null){
            ...
            log.debug("Last bet on course : {}", lastParis.getCourse().getCourseID());
            ...
            if( lastParis.getIsEnded()) {
                ...
                log.info("{} looses, {} place of Martingale, Coef define on {}", count, count % martingale.size(), coefBet);

            }else{
                log.warn("!!! Last Paris not ended (cur: {}, last: {}) !!!!", course.getCourseID(), lastParis.getCourse().getCourseID());
            }

        }else{
            log.warn("Last Paris is null!");
        }
        ...
        log.info("New bet on c{}, {}€ on N°{}", paris.getCourse().getCourseID(), paris.getMise(), numCheval);

        try {
            log.debug("Launching webAction");
            ...
            log.error("Error occurred during WebAction : {}",e.getMessage());
        ...
        if(optParis.isPresent() ){
            ...
            if (true == paris.getIsWin()){
                ...
                if( paris.getGain() == null ){
                    log.warn("!!!!! Error in parsing course arrival for {}... unable to retreive Gain !!!", course.getCourseID());
                }
            }
            ...
            log.warn("!!! Unexisting bet for course {}!!!", course.getCourseID());
        }
